...
| v1.1 | 2026-05-08 | 新增 TOPO-010 跨站手动连线管理（添加/删除/聚合/持久化） | AmpCon Product Team |

---

## 附录 A：跨站手动连线管理 (TOPO-010)

### A.1 功能说明

**功能描述：** 支持用户手动创建、查看、删除跨站点设备间的链路连接。手动创建的链路以紫色虚线区分于系统自动发现的链路（青色实线），支持同设备对多条链路自动聚合显示，支持 localStorage 持久化存储。

**触发条件：** 用户点击工具栏"✎ 添加"按钮打开手动连线对话框。

### A.2 手动连线添加

#### A.2.1 添加对话框

点击工具栏"✎"按钮弹出"手动连线"对话框，表格形式填写连线信息：

| 列名 | 类型 | 说明 |
|------|------|------|
| 源Site | 下拉选择 | 选择源端站点 |
| 源设备 | 下拉选择 | 选择源端设备（依赖源Site选择） |
| 源接口 | 下拉选择 | 选择源端端口（依赖源设备选择） |
| → | 箭头指示 | 方向指示符 |
| 目标Site | 下拉选择 | 选择目标站点 |
| 目标设备 | 下拉选择 | 选择目标设备（依赖目标Site选择） |
| 目标接口 | 下拉选择 | 选择目标端口（依赖目标设备选择） |

- 支持"+ 添加连线"按钮增加多行，一次性创建多条链路
- 每行可通过红色"-"按钮删除
- 下拉框级联：选择 Site 后过滤该 Site 下的设备，选择设备后显示可用端口
- 点击"确认"按钮创建所有有效行的链路

#### A.2.2 添加对话框列表字段

| 字段名 | 中文名 | 说明 | 显示/隐藏 | 列固定 | 列排序 | 对齐方式 | 默认排序 |
|--------|--------|------|-----------|--------|--------|----------|----------|
| fromSiteId | 源Site | 源端站点选择 | 显示 | 否 | 1 | 左对齐 | — |
| fromDeviceId | 源设备 | 源端设备选择（级联） | 显示 | 否 | 2 | 左对齐 | — |
| fromPort | 源接口 | 源端端口选择（级联） | 显示 | 否 | 3 | 左对齐 | — |
| toSiteId | 目标Site | 目标站点选择 | 显示 | 否 | 4 | 左对齐 | — |
| toDeviceId | 目标设备 | 目标设备选择（级联） | 显示 | 否 | 5 | 左对齐 | — |
| toPort | 目标接口 | 目标端口选择（级联） | 显示 | 否 | 6 | 左对齐 | — |

### A.3 手动连线显示

#### A.3.1 折叠态显示（Site 卡片未展开）

- 同一站点对之间的所有手动链路聚合为一条紫色虚线
- 线上标签显示成员数量（如 "×2"、"×3"）
- 与系统 FABRIC 链路并行显示，通过垂直偏移避免重叠（偏移步长 18px）
- 点击聚合线 → 左侧显示 "Selected Link" + Members 列表（该站点对的所有手动链路成员）

#### A.3.2 展开态显示（Site 卡片已展开）

- 同一设备对之间的多条手动链路聚合为一条加粗紫色虚线
- 聚合线上显示成员数量标签（"×N"）
- 单条链路显示为普通紫色虚线
- 点击展开态的手动链路 → 左侧显示 "Connectivity between" 链路详情（和站内链路点击行为一致）

#### A.3.3 视觉区分

| 属性 | 系统自动发现链路 | 手动创建链路 |
|------|------------------|--------------|
| 颜色 | 青色 (#0ABAB5) | 紫色 (#6366f1) |
| 线型 | 实线 | 虚线 (8,4) |
| 标签 | 延迟值 (如 24ms) | 成员数量 (如 ×2) |
| 可删除 | 否 | 是 |
...
- 鼠标悬停在手动链路上时，线缆中间出现红色圆形 × 删除按钮
- 点击删除按钮弹出确认对话框

#### A.4.2 删除确认对话框

对话框以表格形式清晰展示即将删除的链路信息：

| 列名 | 说明 |
|------|------|
| 本端站点 | 源端站点名称（同设备对合并显示） |
| 本端设备 | 源端设备名称（同设备对合并显示） |
| 本端链路 | 源端端口（逐行列出） |
| → | 方向箭头 |
| 对端站点 | 目标站点名称（同设备对合并显示） |
| 对端设备 | 目标设备名称（同设备对合并显示） |
| 对端链路 | 目标端口（逐行列出） |

- 相同设备对的多条链路：站点和设备列用 rowSpan 合并，链路列逐行显示
- 不同设备对之间：加粗分隔线区分
- 按钮："取消" + "删除全部 N 条"（红色）
...
| 字段名 | 中文名 | 说明 | 显示/隐藏 | 列固定 | 列排序 | 对齐方式 | 默认排序 |
|--------|--------|------|-----------|--------|--------|----------|----------|
| fromSiteName | 本端站点 | 源端站点名称 | 显示 | 否 | 1 | 左对齐 | — |
| fromDeviceName | 本端设备 | 源端设备名称 | 显示 | 否 | 2 | 左对齐 | — |
| fromPort | 本端链路 | 源端端口 | 显示 | 否 | 3 | 左对齐 | — |
| toSiteName | 对端站点 | 目标站点名称 | 显示 | 否 | 4 | 左对齐 | — |
| toDeviceName | 对端设备 | 目标设备名称 | 显示 | 否 | 5 | 左对齐 | — |
| toPort | 对端链路 | 目标端口 | 显示 | 否 | 6 | 左对齐 | — |

### A.5 聚合链路逻辑

| 场景 | 聚合规则 | 显示效果 |
|------|----------|----------|
| 折叠态 | 按站点对聚合（fromSiteId + toSiteId） | 一条紫色虚线 + ×N 标签 |
| 展开态 | 按设备对聚合（fromDeviceId + toDeviceId） | 一条加粗紫色虚线 + ×N 标签 |
| 单条链路 | 不聚合 | 普通紫色虚线 |

### A.6 数据持久化

- 手动创建的链路存储在浏览器 `localStorage`（key: `ampcon-manual-links`）
- 页面刷新后自动恢复
- 数据格式：`ManualLink[]`（JSON 序列化）

### A.7 ManualLink 数据结构

```typescript
interface ManualLink {
  id: string;           // 唯一标识（manual-{timestamp}-{random}）
  fromSiteId: string;   // 源站点 ID
  fromDeviceId: string; // 源设备 ID
  fromPort: string;     // 源端口
  toSiteId: string;     // 目标站点 ID
  toDeviceId: string;   // 目标设备 ID
  toPort: string;       // 目标端口
  bandwidth?: string;   // 带宽（可选）
  type: 'fiber' | 'ethernet'; // 链路类型
}
```

### A.8 交互逻辑闭环

| 站点卡片状态 | 点击手动链路 | 左侧抽屉显示 |
|--------------|--------------|--------------|
| 都折叠 | 点击站点对聚合线 | Selected Link → Members（该站点对所有手动链路成员） |
| 都展开 | 点击设备对聚合线 | Connectivity between（链路详情，和站内链路一致） |
| 一折一展 | 点击站点对聚合线 | Selected Link → Members |

### A.9 与 OverviewDrawer 集成

- `manualLinks` 通过 props 传递给 OverviewDrawer
- OverviewDrawer 内部将手动链路转换为 `SiteNeighborLink` 格式
- 与系统 `SITE_NEIGHBOR_LINKS` 合并后按条件过滤显示
- 当 `selectedInterSiteLink.manualLinkIds` 存在时，仅显示指定的手动链路成员
- 当 `selectedInterSiteLink.manualLinkIds` 不存在时，显示该站点对的所有链路（系统 + 手动）
