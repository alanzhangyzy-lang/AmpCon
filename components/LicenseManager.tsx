...
const MOCK_LICENSES = [
  { id: 'LIC-2024-00128', sysname: 'DC-Core-01', ip: '10.1.1.1', switchSN: 'SN-A1B2C3', version: 'PicOS 4.6.1', hwid: '0A:1B:2C:3D:4E:5F', expiryDate: '2026-12-15', activationTime: '2025-12-15', totalDays: 365, remainingDays: 217, status: 'activated' as const, type: 'Subscription', group: 'DC-Fabric-1' },
  { id: 'LIC-2024-00129', sysname: 'DC-Leaf-03', ip: '10.1.2.3', switchSN: 'SN-D4E5F6', version: 'PicOS 4.6.1', hwid: '2C:3D:4E:5F:6A:7B', expiryDate: '2026-06-20', activationTime: '2025-06-20', totalDays: 365, remainingDays: 39, status: 'expiring_soon' as const, type: 'Subscription', group: 'DC-Fabric-1' },
  { id: 'LIC-2024-00130', sysname: 'DC-Spine-02', ip: '10.1.1.2', switchSN: 'SN-G7H8I9', version: 'PicOS 4.5.2', hwid: '4E:5F:6A:7B:8C:9D', expiryDate: '2027-03-01', activationTime: '', totalDays: 0, remainingDays: 0, status: 'not_activated' as const, type: 'Perpetual', group: 'DC-Fabric-2' },
  { id: 'LIC-2024-00131', sysname: 'DC-Leaf-07', ip: '10.1.3.7', switchSN: 'SN-J1K2L3', version: 'PicOS 4.6.0', hwid: '6A:7B:8C:9D:AE:BF', expiryDate: '2025-04-30', activationTime: '2024-04-30', totalDays: 365, remainingDays: 0, status: 'expired' as const, type: 'Subscription', group: 'DC-Fabric-1' },
  { id: 'LIC-2024-00132', sysname: 'DC-Core-02', ip: '10.1.1.3', switchSN: 'SN-M4N5O6', version: 'PicOS 4.6.1', hwid: '8C:9D:AE:BF:C0:D1', expiryDate: '2027-01-10', activationTime: '2026-01-10', totalDays: 365, remainingDays: 243, status: 'activated' as const, type: 'Subscription', group: 'DC-Fabric-2' },
  { id: 'LIC-2024-00133', sysname: 'DC-Leaf-12', ip: '10.1.4.12', switchSN: 'SN-P7Q8R9', version: 'PicOS 4.5.2', hwid: 'AE:BF:C0:D1:E2:F3', expiryDate: '2026-08-22', activationTime: '2025-08-22', totalDays: 365, remainingDays: 102, status: 'activated' as const, type: 'Perpetual', group: 'DC-Fabric-2' },
  { id: 'LIC-2024-00134', sysname: 'DC-Border-01', ip: '10.1.5.1', switchSN: 'SN-S1T2U3', version: 'PicOS 4.6.1', hwid: 'C0:D1:E2:F3:04:15', expiryDate: '2026-11-05', activationTime: '2025-11-05', totalDays: 365, remainingDays: 177, status: 'activated' as const, type: 'Subscription', group: 'DC-Fabric-1' },
  { id: 'LIC-2024-00135', sysname: 'DC-Leaf-15', ip: '10.1.4.15', switchSN: 'SN-V4W5X6', version: 'PicOS 4.6.0', hwid: 'E2:F3:04:15:26:37', expiryDate: '2026-05-28', activationTime: '2025-05-28', totalDays: 365, remainingDays: 16, status: 'expiring_soon' as const, type: 'Subscription', group: 'DC-Fabric-2' },
];
...
const ActionMenu: React.FC<{ status: LicenseStatus; licId: string; onViewDetails?: () => void }> = ({ status, licId, onViewDetails }) => {
  ...
  const actions = [
    { label: 'View Details', icon: Eye, show: true, color: '#64748b', onClick: () => { setOpen(false); onViewDetails?.(); } },
    { label: 'Renew', icon: RefreshCw, show: status === 'expiring_soon' || status === 'expired', color: '#3b82f6', onClick: () => setOpen(false) },
    { label: 'Transfer', icon: RotateCcw, show: status === 'activated' || status === 'expiring_soon', color: '#6366f1', onClick: () => setOpen(false) },
    { label: 'Remove', icon: Trash2, show: true, color: '#ef4444', onClick: () => setOpen(false) },
  ].filter(a => a.show);
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={(e) => { e.stopPropagation(); setOpen(!open); }} style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', color: '#64748b', fontSize: '11px' }}>
      ...
      {open && (<>
        ...
        <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: '4px', background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 20, minWidth: '140px', overflow: 'hidden' }}>
          {actions.map((action, i) => { const Icon = action.icon; return (
            <button key={action.label} onClick={action.onClick} style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%', padding: '8px 12px', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: '12px', color: action.color, fontWeight: 500, textAlign: 'left', borderBottom: i < actions.length - 1 ? '1px solid #f8fafc' : 'none' }}>
      ...
      <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead><tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
            {['Sysname','IP Address','Hardware ID','Version','Status','Validity','Expiry','Action'].map(h => <th key={h} style={{ padding: '12px 14px', textAlign: 'left', fontWeight: 700, color: '#475569', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>)}
          </tr></thead>
          <tbody>{filtered.map((lic, idx) => (
            <tr key={lic.id} style={{ borderBottom: idx < filtered.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
              <td style={{ padding: '12px 14px', fontWeight: 600, color: '#1e293b' }}>{lic.sysname}</td>
              <td style={{ padding: '12px 14px', color: '#64748b', fontFamily: 'monospace', fontSize: '11px' }}>{lic.ip}</td>
              <td style={{ padding: '12px 14px', color: '#64748b', fontFamily: 'monospace', fontSize: '11px' }}>{lic.hwid}</td>
              <td style={{ padding: '12px 14px', color: '#64748b' }}>{lic.version}</td>
              ...
              <td style={{ padding: '12px 14px', color: '#64748b', fontSize: '11px' }}>{lic.expiryDate}</td>
...
const OnlineActivateTab: React.FC = () => {
  ...
  const unactivated = MOCK_LICENSES.filter(l => l.status === 'not_activated' || l.status === 'expired');
  const toggleSelect = (id: string) => setSelectedDevices(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  const handleBatchActivate = () => {
    setActivating(true); setProgress(0); setActivationLog([]);
    const iv = setInterval(() => { setProgress(p => { if (p >= 100) { clearInterval(iv); setActivating(false); setActivationLog([{device:'DC-Spine-02',status:'success',msg:'License activated'},{device:'DC-Leaf-07',status:'failed',msg:'No license found on Portal'}]); return 100; } return p + 25; }); }, 500);
  };
  return (
    <div>
      {/* Portal Connection Status */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', background: portalStatus === 'connected' ? '#ecfdf5' : '#fef2f2', borderRadius: '12px', marginBottom: '20px', border: `1px solid ${portalStatus === 'connected' ? '#a7f3d0' : '#fecaca'}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {portalStatus === 'connected' ? <Wifi size={20} color="#10b981" /> : <WifiOff size={20} color="#ef4444" />}
          <div>
            <p style={{ fontSize: '13px', fontWeight: 700, color: portalStatus === 'connected' ? '#065f46' : '#991b1b' }}>{portalStatus === 'connected' ? 'License Portal Connected' : 'License Portal Disconnected'}</p>
            <p style={{ fontSize: '11px', color: portalStatus === 'connected' ? '#047857' : '#b91c1c', marginTop: '2px' }}>{portalStatus === 'connected' ? 'Latency: 45ms | API v2.1 | Last sync: 2 min ago' : 'Check System Configuration for API credentials'}</p>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button style={{ padding: '6px 12px', borderRadius: '8px', border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontSize: '11px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px', color: '#3b82f6' }}><ExternalLink size={12} /> Open Portal</button>
        </div>
      </div>
      {/* Batch Activate */}
      <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={() => setSelectedDevices(unactivated.map(d => d.id))} style={{ padding: '6px 12px', borderRadius: '8px', border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontSize: '11px', fontWeight: 600 }}>Select All</button>
        ...
        {activating && (<div style={{ marginBottom: '16px', padding: '12px 16px', background: '#eff6ff', borderRadius: '8px', border: '1px solid #bfdbfe' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}><span style={{ fontSize: '11px', fontWeight: 600, color: '#1d4ed8' }}>Activating licenses via Portal API...</span><span style={{ fontSize: '11px', fontWeight: 700, color: '#1d4ed8' }}>{progress}%</span></div><div style={{ height: '4px', background: '#dbeafe', borderRadius: '2px', overflow: 'hidden' }}><div style={{ width: `${progress}%`, height: '100%', background: '#3b82f6', borderRadius: '2px', transition: 'width 0.3s' }} /></div></div>)}
        {/* Activation Results Log - inspired by Cisco workflow results */}
        {activationLog.length > 0 && (<div style={{ marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '6px' }}>{activationLog.map((log, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', borderRadius: '6px', background: log.status === 'success' ? '#ecfdf5' : '#fef2f2', fontSize: '12px' }}>{log.status === 'success' ? <CheckCircle2 size={14} color="#10b981" /> : <XCircle size={14} color="#ef4444" />}<span style={{ fontWeight: 600, color: '#1e293b' }}>{log.device}</span><span style={{ color: '#64748b' }}>{log.msg}</span>{log.status === 'failed' && <button style={{ marginLeft: 'auto', padding: '2px 8px', borderRadius: '4px', border: '1px solid #fecaca', background: '#fff', cursor: 'pointer', fontSize: '10px', fontWeight: 600, color: '#ef4444', display: 'flex', alignItems: 'center', gap: '3px' }}><RotateCcw size={10} />Retry</button>}</div>))}</div>)}
        {/* Device List */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {unactivated.map(device => (
            <div key={device.id} onClick={() => toggleSelect(device.id)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderRadius: '8px', border: `1px solid ${selectedDevices.includes(device.id) ? '#3b82f6' : '#e2e8f0'}`, background: selectedDevices.includes(device.id) ? '#eff6ff' : '#fff', cursor: 'pointer', transition: 'all 0.15s' }}>
              <input type="checkbox" checked={selectedDevices.includes(device.id)} readOnly style={{ accentColor: '#3b82f6' }} />
              <div style={{ flex: 1 }}><span style={{ fontSize: '13px', fontWeight: 600, color: '#1e293b' }}>{device.sysname}</span><span style={{ fontSize: '11px', color: '#94a3b8', marginLeft: '12px' }}>HWID: {device.hwid}</span></div>
              <span style={{ fontSize: '11px', color: '#64748b', fontFamily: 'monospace' }}>{device.ip}</span>
...
const ImportLicenseTab: React.FC = () => {
  ...
  const preImportPool = [{ id: 'pool-1', hwid: 'FF:00:11:22:33:44', fileName: 'license_FF0011.lic', uploadTime: '2026-05-10 14:30' }, { id: 'pool-2', hwid: 'AA:BB:CC:DD:EE:FF', fileName: 'license_AABBCC.lic', uploadTime: '2026-05-08 09:15' }];
  return (
    <div>
      {/* Single Add Section */}
      <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '16px 20px', marginBottom: '16px' }}>
      ...
      {/* Batch Upload Area */}
      <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop} style={{ border: `2px dashed ${dragOver ? '#3b82f6' : '#e2e8f0'}`, borderRadius: '12px', padding: '32px', textAlign: 'center', background: dragOver ? '#eff6ff' : '#fafafa', marginBottom: '20px', cursor: 'pointer', transition: 'all 0.2s' }}>
      ...
      {/* Import Results */}
      {importResults.length > 0 && (<div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '16px', marginBottom: '20px' }}>
      ...
      <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '16px' }}>
        ...
        <p style={{ fontSize: '11px', color: '#94a3b8', marginBottom: '12px' }}>Licenses waiting for device registration. Auto-activates when a matching HWID device registers.</p>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}><thead><tr style={{ borderBottom: '1px solid #f1f5f9' }}>{['HWID','File','Upload Time','Status',''].map(h => <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 700, color: '#475569', fontSize: '11px' }}>{h}</th>)}</tr></thead><tbody>{preImportPool.map(item => (<tr key={item.id} style={{ borderBottom: '1px solid #f8fafc' }}><td style={{ padding: '10px 12px', fontFamily: 'monospace', color: '#1e293b', fontSize: '11px' }}>{item.hwid}</td><td style={{ padding: '10px 12px', color: '#64748b' }}>{item.fileName}</td><td style={{ padding: '10px 12px', color: '#64748b' }}>{item.uploadTime}</td><td style={{ padding: '10px 12px' }}><span style={{ padding: '2px 8px', borderRadius: '12px', fontSize: '10px', fontWeight: 700, color: '#2563eb', background: '#eff6ff' }}>Pending</span></td><td style={{ padding: '10px 12px', textAlign: 'center' }}><button style={{ padding: '4px', borderRadius: '4px', border: 'none', background: 'transparent', cursor: 'pointer', color: '#ef4444' }}><Trash2 size={14} /></button></td></tr>))}</tbody></table>
      </div>
    </div>
  );
};
...
const LicenseManager: React.FC = () => {
  ...
  const tabs: { id: TabType; label: string; sublabel: string }[] = [
    ...
    { id: 'online_activate', label: 'Online Activate', sublabel: 'Quick Activate via Portal' },
    { id: 'import', label: 'Import License', sublabel: 'Local import & pre-import pool' },
  ];
  return (
    <div style={{ padding: '24px 32px', background: '#ffffff', minHeight: '100vh', overflow: 'auto' }}>
      ...
      {/* Tab Navigation */}
      <div style={{ display: 'flex', gap: '2px', marginBottom: '24px', background: '#e2e8f0', borderRadius: '10px', padding: '3px' }}>
      ...
      {activeTab === 'online_activate' && <OnlineActivateTab />}
      {activeTab === 'import' && <ImportLicenseTab />}
    </div>
  );
};
